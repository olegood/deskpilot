...
from deskpilot.db.models import OrderStatus, UserRole
...
@pytest.fixture
async def as_noah(seeded_sessions: async_sessionmaker[AsyncSession]) -> AgentContext:
    return AgentContext(
...
async def test_get_customer_follows_the_context_not_an_argument(
    ...
    as_ana = AgentContext(
        principal=await principal_for(seeded_sessions, ANA), session_factory=seeded_sessions
    )
...
async def test_list_orders_refuses_a_principal_who_owns_no_customer_record(
    seeded_sessions: async_sessionmaker[AsyncSession],
) -> None:
    """Staff own no orders, so the scope check refuses before any query runs."""
    staff = AgentContext(
        principal=Principal(
            user_id=99,
    ...
    message = await invoke_tool(list_orders, staff)
...
async def test_list_orders_admits_when_it_has_truncated(
    ...
    cramped = AgentContext(
        principal=await principal_for(seeded_sessions, NOAH),
        session_factory=seeded_sessions,
